...
# version. The event name arrives as $1; SubagentStart coverage exists so
# subagent and worker transcripts carry the stamp too.
#
...
if [[ -L "$claude_md" ]]; then
    ...
    if [[ -n "$repo_dir" ]]; then
        ...
        if [[ -n "$sha" ]]; then
            # A dirty checkout is not a clean instruction version; mark it.
            dirty=""
