...
Conventions for structuring a backend service — or any module that owns persistence and external I/O. Adapt the layer names to your stack (HTTP/RPC framework, ORM); the structure is what matters. For general TypeScript style (types, naming, null handling), see the `typescript-clean-code` skill.
...
Keep the layering pragmatic: a trivial read-through endpoint may call a repository directly.
...
- **`repository.ts`**: queries, explicit return types, and a required `database` parameter that also accepts a transaction.
- **`service.ts`**: orchestrates a `$transaction`, threading the `tx` into repository calls so related writes commit atomically; plus an ownership check that returns a typed `kind` result instead of throwing.
- **`controller.ts`**: transport only. It validates input, requires authentication, delegates, and maps each error `kind` to a transport error. It runs no queries and holds no business rules.
