...
```bash
cd <path-to-this-skill-directory> && bun install
```
...
```bash
bun <path-to-this-skill-directory>/fetch.ts "<url>"
```
...
**`bun` or script deps missing**: Run `bun install` from this skill directory.
