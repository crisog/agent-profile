...
Nothing you run may block on a prompt.
...
- Push, force-push, deploy, publish, merge.
...
checks. For wider work, derive scope from committed artifacts: specs,
recent commit messages. Keep working independent items when a boundary blocks
one action. Before stopping, save the outcomes and unfinished state to a PR
body, tracker comment, or persistent memory.
