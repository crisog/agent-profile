...
**Test traceability** (conditional) -- `REQ-*` to test file:line mapping. Added during/after the failure-modes gate, not at initial authoring. The mapped test asserts the requirement, not the code that implements it -- a test traceable only to a call sequence proves the implementation exists, not that the requirement holds.
