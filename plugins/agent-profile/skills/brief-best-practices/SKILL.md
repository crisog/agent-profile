...
- `example-payments.md` — a filled brief for a money-transfer flow: the **objective** archetype, where floors are machine-checkable and the oracle is property tests + staging. `example-taste.md` — the **subjective/taste** archetype: a visual or design surface whose bar is "elegant, calm, recognizable" and whose oracle is a blind human-judge quorum (maker ≠ judge) closing on a culminating "Final acceptance" frame — is the harder, more common product case; build its oracle from the blind-judge pattern in the Oracle slot.
